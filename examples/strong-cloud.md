# Strong-cloud example

This example shows a project that wants strong public reciprocity for hosted use,
cloud deployment artifacts, anti-shim rules, private-fork deadlines, and CLA
infrastructure.

## Good fit

- Hosted infrastructure or control-plane software
- Projects worried about shell publication and private fork cycling

## Bad fit

- Projects that want a simpler community posture
- Projects that do not want CLA administration

## Filled Annex A

```text
====================================================================
LICENSE ACTIVATION BLOCK
Modular Cooperative License v1.0
====================================================================

Software Name:
Atlas Cluster

Version or Release Identifier:
2026.2

Copyright Holder(s):
Atlas Cluster Systems, Inc.

Contact for Legal Notices:
legal@atlascluster.example

Primary Public Repository:
https://source.example/atlas-cluster/core

Governing Law:
New York, United States

Notification Method for MOD-PUB:
[ ] Issue or pull request in the primary repository
[x] Email or web form identified below
[ ] Other durable written method described below
Details:
Email notices to pub-notice@atlascluster.example.

ACTIVATED MODULES
[ ] MOD-AI     Machine Learning Training Restriction
[x] MOD-BIN    Cloud Binary and Deployment Release
[x] MOD-CLA    Contributor License Agreement and Relicensing
[ ] MOD-COM    Commercial Non-Competing Restriction
[x] MOD-DIST   Distribution Copyleft
[x] MOD-INT-A  Internal Use, Permissive Variant
[ ] MOD-INT-B  Internal Use, Reciprocal Variant
[ ] MOD-INT-C  Internal Use, Public Covered-Work Release Variant
[ ] MOD-INT-D  Internal Use, Public Integrated-System Release Variant
[x] MOD-NET    Network and SaaS Copyleft
[x] MOD-PLUG   Plugin, Boundary, and Anti-Shim Enforcement
[x] MOD-PUB    Public Fork Requirement
[ ] MOD-SUB    Sublicensing Permission

MOD-COM CONFIGURATION
Primary Commercial Function:
Not applicable because MOD-COM is inactive.

MOD-PLUG CONFIGURATION
Documented Extension API Location:
https://developer.atlascluster.example/extensions

Integral Component Designations Beyond the Default Definition:
See INTEGRAL_COMPONENTS.md

MOD-CLA CONFIGURATION
CLA Location:
https://atlascluster.example/legal/cla

Permitted Relicensing Scope:
Future versions of MCL published by the project steward.

ADDITIONAL PROJECT-SPECIFIC CLARIFICATIONS
Compliance records and deployment artifacts for public hosted releases are
published from the primary repository release page.
```
